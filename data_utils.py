...
def load_and_convert_h36m_to_3d_and_save(path_to_dataset, subjects, actions, sample_rate, seq_len):
    """
    继承自load_data_3d函数的代码
    adapted from
    https://github.com/una-dinosauria/human-motion-prediction/src/data_utils.py#L216
    :param path_to_dataset:
    :param subjects:
    ...
    sampled_seq = []
    complete_seq = []
    for subj in subjects: #演员
        for action_idx in np.arange(len(actions)): #动作
            action = actions[action_idx] #第几次表演
            for subact in [1, 2]:  # subactions
                ...
                action_sequence = readCSVasFloat(filename)
                n, d = action_sequence.shape
                even_list = range(0, n, default_sample_rate)
                num_frames = len(even_list)
                the_sequence = np.array(action_sequence[even_list, :])
                the_seq = Variable(torch.from_numpy(the_sequence)).float().cuda()
                # remove global rotation and translation
