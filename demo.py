...
def main(opt):
    ...
    if is_cuda:
        model.cuda()
    model_path_len = './checkpoint/test/ckpt_main_in10_out25_dctn35_best.pth.tar'
    ...
    for act in acts:
        for i, (inputs, targets, all_seq) in enumerate(test_data[act]):
            ...
            for k in range(2):
                plt.cla()
                figure_title = "{},{},".format(act, (k + 1))
                viz.plot_predictions(targ_expmap[k, :, :], pred_expmap[k, :, :], fig, ax, figure_title, first)
                first = False
                plt.pause(0.8)
