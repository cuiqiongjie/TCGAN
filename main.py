...
def main(opt):
    ...
    print("using cuda is {}".format(is_cuda))
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        # per epoch
        lr_now, t_l, t_e, t_3d = train(epoch, train_loader, model, optimizer, input_n=input_n,
...
def train(epoch, train_loader, model, optimizer, input_n=20, dct_n=20, lr_now=None, max_norm=True, is_cuda=False, dim_used=[]):
    t_l = utils.AccumLoss()
    t_e = utils.AccumLoss()
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        if is_cuda:
            ...
            all_seq = all_seq.to(device).float()
        ...
        if i%10 is 0:
            print("{}-th epoch, {}-th update".format(epoch, i))
