...
def main(opt):
    ...
    for epoch in range(start_epoch, opt.epochs):
        ...
        head = np.array(['epoch'])
        # per epoch
        lr_now, t_l, t_e, t_3d = train(train_loader, model, optimizer, epoch, input_n=input_n, lr_now=lr_now,
...
def train(train_loader, model, optimizer,epoch_num, input_n=20, lr_now=None, max_norm=True, is_cuda=False, dim_used=[], dct_n=20):
    t_l = utils.AccumLoss()
    t_e = utils.AccumLoss()
    ...
    for i, (inputs, targets, all_seq) in enumerate(train_loader):
        ...
        if i%10 is 0:
            print("{}-th epoch, {}-th update".format(epoch_num, i))
