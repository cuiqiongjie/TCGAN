...
class Ax3DPose(object):
    def __init__(self, ax, lcolor="#3498db", rcolor="#3498db", label=['GT', 'Pred']): """
    ...
    for i in np.arange(len(self.I)):
        ...
        if i == 0:
            self.plots.append(
                self.ax.plot(x, z, y, lw=2, linestyle='--', label=label[0], marker='.', markersize=6,  color='#8e8e8e') )
        else:
            self.plots.append(self.ax.plot(x, y, z, lw=2, linestyle='--', marker='.', markersize=6, color='#8e8e8e'))
    ...
    for i in np.arange(len(self.I)):
        ...
        z = np.array([vals[self.I[i], 2], vals[self.J[i], 2]])
        if i == 0:
            self.plots_pred.append(self.ax.plot(x, y, z, lw=2, c=rcolor if self.LR[i] else lcolor, label=label[1], marker='.',  markersize=6, color='#3498db'))
        else:
            self.plots_pred.append(self.ax.plot(x, y, z, lw=2, c=rcolor if self.LR[i] else lcolor, marker='.', markersize=6, color='#3498db'))
...
def plot_predictions(expmap_gt, expmap_pred, fig, ax, f_title, is_first=False):
    # Load all the data
    ...
    for i in range(nframes_pred):
        ...
        fig.canvas.draw()
        if i <10:
            ax.set_title('Observed motion:'+ f_title + ' frame:{:d}'.format(i + 1), loc='center')
        else:
            ax.set_title('Prediction:' + f_title + ' frame:{:d}, time:({}ms)'.format(i + 1-10, (i + 1-10) * 40), loc='center')
        plt.pause(0.05)
